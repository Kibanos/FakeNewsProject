...
# gdb = GradientBoostingClassifier(n_estimators=200, random_state=14128, verbose=True)
...
# #Load the training dataset and generate folds
# d = DataSet(path="fnc_1")
# ss=ShuffleSplit(test_size=0.2,train_size=0.8,random_state=1489215)
# article_ids = list(d.articles.keys())
# article_ids = np.array([int(i) for i in article_ids])
# best_score=0
# best_fold=None
# for train_index, test_index in ss.split(article_ids, d.stances):
#     folds,hold_out =article_ids[train_index],article_ids[test_index]
#     fold_stances, hold_out_stances = d.stances[train_index],d.stances[test_index]
#     X_holdout,y_holdout = generate_features(hold_out_stances,d,"holdout")
#     kf=KFold(n_splits=10)
#     sub_score=0
#     clf=RandomForestClassifier(n_estimators=200,random_state=14128,n_jobs = -1)
#     for train, test in kf.split(folds,fold_stances):
#         F_train,F_test=folds[train],folds[test]
#         S_train,S_test=fold_stances[train],fold_stances[test]
#         # Load/Precompute all features now
#         X_test,y_test = generate_features(fold_stances[test],d,str(test)[1:3])
#         X_train,y_train = generate_features(fold_stances[train],d,str(train)[1:3])
#
#         clf.fit(X_train, y_train)
#         predicted = [LABELS[int(a)] for a in clf.predict(X_test)]
#         actual = [LABELS[int(a)] for a in y_test]
...
# # kn2=KNeighborsClassifier()
# # nb=GaussianNB()
# # svm1=LinearSVC(random_state=14128,multi_class='crammer_singer')
# # svm2=LinearSVC(random_state=14128)
# # lr1=LogisticRegression(random_state=14128,multi_class='multinomial',solver='newton-cg')
# # lr2=LogisticRegression(penalty='l1')
# # sdg=SGDClassifier(random_state=14128,n_jobs= -1)
# # nn=MLPClassifier(activation='identity',solver='sgd',learning_rate='adaptive',random_state=14128)
# # gdb = GradientBoostingClassifier(n_estimators=200, random_state=14128, verbose=True)
# # param=[sdg,nn,gdb]
# # # for each system
# # p=8
...
# #         # clf = GridSearchCV(lr, LRparams)
# #         #
# #         # SDGparams={'loss':['hinge','log','modified_huber','squared_hinge','perceptron','epsilon_insessitive','huber','squared_loss'],'penalty':[None,'l2','l1','elasticnet'], 'fit_intercept':[True,False],'random_state':[None,14128],'learning_rate':['constant','optimal','invscaling'],'eta0':[0.0,0.1,0.5,1.0],'average':[True,False]}
# #         # sdg=SGDClassifier()
# #         # clf = GridSearchCV(sdg, SDGparams)
# #         #
# #         # NNparams={'hidden_layer_size':[],'activation':['identity','logistic','tanh','relu'],'solver':['lbfgs','sgd'],'learning_rate':['constant','invscaling','adaptive'],'random_state':[None,14128],'early_stopping':[False,True]}
# #         # nn=MLPClassifier()
# #         # clf = GridSearchCV(nn, NNparams)
# #         #
# #         # NBparams={}
...
# #         print("Score for fold "+ str(fold) + " was - " + str(score))
# #         best_score +=score
# #     if best_score/len(fold_stances)>sub_score:
# #         sub_score=best_score/len(fold_stances)
# #         best_fold = clf
# #         joblib.dump(clf, 'fold'+str(p)+'.pkl')
# #
#
